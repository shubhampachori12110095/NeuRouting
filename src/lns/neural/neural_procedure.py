...
from environments.batch_lns_environment import BatchLNSEnvironment
        ...
        for epoch in range(n_epochs):
            for batch_idx in range(n_batches):
                ...
                if val_instances is not None:
                    val_steps = val_steps if val_steps is not None else val_instances[0].n_customers
                ...
                if val_instances is not None and (batch_idx + 1) % val_interval == 0 or batch_idx + 1 == n_batches:
                    ...
                    solutions = self.validate(opposite_procedure, val_instances, batch_size, val_steps)
    ...
    def validate(self,
        ...
        if self.val_env is None:
            if isinstance(opposite_procedure, RepairProcedure) and isinstance(self, DestroyProcedure):
                base_env = LNSEnvironment(operators=[LNSOperator(self, opposite_procedure)], neighborhood_size=batch_size)
            elif isinstance(opposite_procedure, DestroyProcedure) and isinstance(self, RepairProcedure):
                base_env = LNSEnvironment(operators=[LNSOperator(opposite_procedure, self)], neighborhood_size=batch_size)
            else:
                base_env = None
        ...
        return self.val_env.solve(val_instances, max_steps=val_steps)
